...
function renderHeader(report: ConformanceReport): string {
  ...
  if (report.server.instructions) {
    // Quote single-line snippets; truncate multi-line ones so the
    // header stays scannable.
    const flat = report.server.instructions.replace(/\s+/g, " ").trim();
    const snippet = flat.length > 120 ? flat.slice(0, 117) + "..." : flat;
...
function renderFindingsSummary(findings: Finding[]): string {
  ...
  if (findings.length === 0) {
    lines.push("No lint findings — every tool's schema passes the conformance rules.");
    return lines.join("\n");
  }
...
function formatFuzzRow(r: ConformanceReport["fuzz"][number]): string {
  ...
  const note = (() => {
    if (r.outcome === "toolError" && r.errorMessage) {
      return truncate(r.errorMessage, 60);
    }
    if (r.outcome === "protocolCrash" && r.errorMessage) {
      return `crash: ${truncate(r.errorMessage, 50)}`;
    }
    return "";
  })();
...
/** Escape `|` so it doesn't break the Markdown table column. */
